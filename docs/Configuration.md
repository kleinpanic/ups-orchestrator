...
| `serial`, **armed** | Without `--deep`: checks the recorded device's presence (a stat, injected in tests). With `--deep`: additionally asks the far end to execute a probe and echo a token back on the same line | Shallow: rc 0 if present and a character device with a usable declared baud, rc 1 otherwise. Deep: rc 0 only if the token came back; see below |
| `serial`, **armed, `--deep`** | Runs the serial liveness probe. This is the only check that produces evidence about the *far end* rather than about the local adapter — a USB serial adapter presents a character device whether or not anything is plugged into its other end, so the shallow `OK` is a statement about the dongle | rc 0 on `SEEN` (the far end executed something — proof of execution, not echo, because the token is written split and matched whole). rc 1 on `NOT_SEEN` or `NO_TRANSPORT`. `NOT_SEEN` is reported as an *observation*: a busy console, a getty that does not echo, and a genuinely wrong baud are indistinguishable from this end, so the output says so rather than claiming the line is dead |
